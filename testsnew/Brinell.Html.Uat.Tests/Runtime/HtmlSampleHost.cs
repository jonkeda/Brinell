...
internal sealed class HtmlSampleHost : IDisposable
{
    ...
    private async Task WaitUntilReadyAsync()
    {
        ...
        while (DateTime.UtcNow < deadline)
        {
            if (_process.HasExited)
            {
                throw new InvalidOperationException(
                    $"HTML sample app exited with code {_process.ExitCode}.{Environment.NewLine}{FormatOutput()}");
            }
        ...
        throw new TimeoutException(
            $"HTML sample app did not become HTTP-ready at {BaseUrl}.{Environment.NewLine}{FormatOutput()}");
    }
