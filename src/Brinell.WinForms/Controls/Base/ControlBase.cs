...
/// <summary>
/// WinForms-specific base class for all controls using FlaUI directly.
...
public abstract class ControlBase : IControlObject
{
    ...
    /// </summary>
    protected void ThrowCheckFailed(string action, string message)
    {
        Logger?.ThrowCheckFailed(TestName, PageName, AutomationId, action, message, _context);
    }
    ...
    /// <summary>
    /// Click the control.
    /// Override in derived classes that support clicking.
    /// </summary>
    ...
    public virtual void SetText(string text)
    {
        ...
        if (element == null)
        {
            ThrowCheckFailed("SetText", $"Element '{AutomationId}' not visible for text input.");
        }
        
        var textBox = element!.AsTextBox();
        if (textBox != null)
        {
            // Clear first by setting Text property directly
            textBox.Text = string.Empty;
            System.Threading.Thread.Sleep(50); // Brief delay to allow UI to process clear
            
            // Set text using Text property assignment instead of Enter() to avoid corruption
            textBox.Text = text;
